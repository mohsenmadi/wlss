...
//		println 'valid payment'
    ...
    def savePayment(amendsToPay, payment, payType,comment) {
        def json = constructJson(amendsToPay, payment, payType,comment)
//        log.info "json for payment...${json}"
        def resp = wsReqService.makePostWsr(wsPostPayUrl, json.toString())

        resp
    }

    def constructJson(amendsToPay, payment, payType,comment) {

        def builder = new JsonBuilder()
        builder.ggPayment {
            username "${liuService.user.username}"
            referKey "${payment.referKey}"
//            comment "${comment?:comment}"  //TODO YB to update
