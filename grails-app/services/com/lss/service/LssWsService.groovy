...
class LssWsService {
	...
	def validatePassword(json) {
		if (json.password && json.password != 'NEW CALL') {
			...
			log.info "== pw changed to ${json.password}"
			return 'changeOfPw'
		}
		'noPw'
	}
	...
	def submitExemptFiling(mapExempt, filingYear, theComment, mapMovedMembers, setSearchMembers) {
		...
		if (!json?.accept) { // happens with 500 errors...
			def relatedTo = "${filingYear}"
			addNotification("Sorry, there are some problems submitting your exemption! Please click reload data.", relatedTo, 1)
			log.debug "=5= $json"
			return [false, null]
		}
	...
	def registerFirmMembers(firm) {
		...
		log.info " == firm has ${size} lawyers"
		def timeTaken = System.currentTimeMillis()

		json.lawyers.eachWithIndex { currLaywer, idx ->
			def lawyer = domainCreateService.registerLawyer(currLaywer.lawyer, firm)

			log.info "  == adding ${idx+1}/${size} $lawyer.username: "
			currLaywer.qtrs.each { filingKey ->
				print "$filingKey  "
				domainCreateService.createNewMemActivity(mapOfFilings[filingKey], lawyer, ActivityOwner.FIRM)
			}
		...
		log.info "==== time taken: ${System.currentTimeMillis() - timeTaken}"
	...
	def updateFilings() {
		...
		liuService.filingYears.filings*.each { filing ->
			if (filingService.isQrEnabled(filing)) {
				log.info "*** updating ${filing.filingYear.year} - ${filing.quarter} ***"
				updateFiling(filing)
			}
		}
		log.info "==== time taken: ${System.currentTimeMillis() - timeTaken}"
...
//		log.info JsonOutput.prettyPrint(json)

//		def timeTaken = System.currentTimeMillis()
		def blAmnd = filingService.getBeforeLastAmendment(filing)
		def activityOwner = liuService.activityOwner
//		log.info "    000blAmnd000 :: ${System.currentTimeMillis() - timeTaken}\t "

		json.amnds.each {

//			timeTaken = System.currentTimeMillis()

			log.info "==the json== $it"
			println "==the json== $it"
...
//			print "4 :: ${System.currentTimeMillis() - timeTaken}\t "
			...
			updatedAmnd = newAmnd
			log.info "   == created amnd ${i++}: $newAmnd" // \t:: ${System.currentTimeMillis() - timeTaken}"
		}
...
//		log.info "    999notify999 :: ${System.currentTimeMillis() - timeTaken}\t "
...
//		log.info JsonOutput.prettyPrint(jsonBuilder.toString())
//		log.info JsonOutput.prettyPrint(wsResponse.text)