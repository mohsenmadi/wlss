...
class FilingController implements Serializable {
	...
	def addAmendment() {
		...
		if (params.submitBtnAction != 'submit') {
			...
			if (newAmndIsSame(newAmnd)) {
				...
				(newAmnd, filing) = lssWsService.submitAmendment(
						filing, lastAmnd,	newAmnd,
						commonService.getComment(params?.txtareaComment),
		...
		render (template:'amendment',
			model:[createAmendment:createAmendment, cbShowTableVal:params.cbShowTable,
...
//		println "=2= ${liuService.firmLawyers.username.sort { it }}"
	...
	def rerenderQuarter() {		
		...
		render (template:'amendment',
			model:[createAmendment:false, firm:liuService.firm,
					 cbShowTableVal:params.cbShowTable,
	...
	def getExListMembers(year) {
		...
		def leftList = exListMembers.findAll {
			it.username.contains('~')
		}
		exListMembers -= leftList
		exListMembers.sort{ it.username }
	}
	
	def rerenderExemption() {
		render (template:'exemptionsTemplate',
			model:[addBtnVal:'start', finCustRole:liuService.liUserIsFinOrCustSrv(),
					 cbShowTableVal:params.cbShowTable,
	...
	def exemptions() {
		...
		[addBtnVal:'start', finCustRole:liuService.liUserIsFinOrCustSrv(), exFy:exFy,
			listMembers:getExListMembers(exDisplayYear), displayYear:exDisplayYear]
	}
	...
	def fileExemption() {
		...
		render (template:'exemptionsTemplate',
			model:[createAmendment:createAmendment, cbShowTableVal:params.cbShowTable,
					...
					addBtnVal:addBtnVal, msgCode:msgCode,
	...
	def fileOrAmend() {
		filing.amendments.size() < 2 ? 'start' : 'amend'
	}
